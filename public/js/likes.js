        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.is_like == 1) {
                    ...
                    $('*[data-postid="'+ post_id + '_l"]').removeClass('btn-danger').addClass('btn-secondry');
                ...
                if (data.is_like == 0) {
                    $('*[data-postid="'+ post_id + '_d"]').removeClass('btn-success').addClass('btn-secondary');
                    var cu_like = $('*[data-postid="'+ post_id + '_l"]').find('.like_count').text();
                    var new_like = parseInt(cu_like) - 1 ;
                    $('*[data-postid="'+ post_id + '_l"]').find('.like_count').text(new_like);
...
$.ajax({
    type : 'POST',
     url : url_dis,  // the first url from js, the second from master which definition im master
    data : { like_s: like_s, post_id: post_id, _token: token },
    ...
    if( data.is_dislike == 0)
    {
        ...
        var cu_dislike = $('*[data-postid="'+ post_id + '_l"]').find('.dislike_count').text();
        var new_dislike = parseInt(cu_dislike) - 1 ;
        $('*[data-postid="'+ post_id + '_d"]').find('.dislike_count').text(new_dislike);
